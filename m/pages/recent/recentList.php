      ...
      <div class="body_div_activity">
        <div class="tag_div_activity">
          <ul class="tag_ul_activity">
            ...
            $index ++;
            ?>
            <li class="tag_type li<?php echo $index ?>">
              <a href="/m/recent/recent_tag.php?id=<?php echo $tag->id ?>" ><?php echo $tag->name; ?></a>
            </li>
            <?php } ?>
          </ul>
        </div>

        <?php foreach($rec_list as $recent_act){?>
        <div class="content_white_activity">
          <div class="content_div_activity">
            ...
            <p class="con_title_type_activity">活动时间</p>
			      <p class="con_summary_activity"><?php $endTime = strtotime($recent_act->start_time);echo date("Y-m-d H:i", $endTime); ?> ~ <?php $startTime = strtotime($recent_act->end_time);echo date('m-d H:i', $startTime); ?></p>
			...
			<a href="javascript:void(0);" class="con_activity_activity">报名已结束</a>
			<?php } ?>
          </div>
        </div>
        <?php } ?>
...
<script type="text/javascript">
  $(".body_type_recent_talk").click(function() {
    ...
    $(".body_div_recent_talk").css('display','block');
    $(".body_div_activity").css('display','none');
  ...
  function loadMessage() {
    ...
    if(num == 0) {
      ...
      num = 1;
      var tag = 0
      var url = "/m/api/recent.php?next_id="+next_id+"&count=10&tag=&mod=talk";
			...
			if(act_list.length != 0) {
				...
				for(var i = 0; i < act_list.length; i ++) {
					html += "<div class='content_white_activity'>"+
						  ...
						  "<p class='con_title_type_activity'>活动介绍</p>"+
						  "<p class='con_summary_activity'>"+act_list[i]['summary']+"</p>"+
						  "<p class='con_title_type_activity'>活动时间</p>"+
					...
					var nowTime = '<?php date_default_timezone_set("Asia/Shanghai"); echo date("Y-m-d");?>';
					if(act_list[i]['allow'] != 0) {
						if(nowTime <= act_list[i]['start_time']) {
                          if(act_list[i]['is_fress'] != 0){
					...
					} else if(nowTime > act_list[i]['start_time']) {
					   	html += "<a href='javascript:void(0);' class='con_activity_activity'>报名已结束</a>";
					}
